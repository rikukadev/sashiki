...
> `branch` / `profile` に使えるのは英数字と `.` `_` `-`(64 文字まで)。それ以外(空白・引用符・`$()` など)は
> 送信前に拒否する。`transport: ssm` は値をインスタンス上のシェルで実行するコマンドに埋めるため、PR の
> ブランチ名のような信頼できない値をそのまま `branch:` に渡す使い方でも安全なように、client 側で検証し、
> 1 引数ずつクォートして送る(#339)。
