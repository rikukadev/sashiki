...
jobs:
  branch:
    ...
    steps:
      - uses: rikukadev/sashiki/action@v0.11.1 # x-release-please-version
        with:
...
### branch / profile に渡せる値

`branch` と `profile` は `^[a-z0-9][a-z0-9-]{0,31}$` に絞り、外れる値は**送信前に**拒否する。
空白・引用符・`$()`・改行・先頭ハイフンはどれも通らない。

サーバー既定の `branches.name_pattern`(`^[a-z0-9-]{1,32}$`)より**先頭ハイフンだけ狭い**。
argv 転送では値がそのまま `sashiki` の引数になるので、`-rf` や `--config` を渡すと
ブランチ名ではなく**フラグとして読まれる**(`unknown flag -rf`)。シェル注入にはならないが、
渡した名前が黙って別の意味になるため入口で止めている。

`transport: ssm` はインスタンス上で高権限のシェルが動く経路なので、PR のブランチ名のような
信頼できない値を `branch:` に渡す使い方を想定して二重に守っている(#339)。

1. client 側で上のパターンに合わない値を拒否する
2. 通った値も**シェル文字列へ連結しない**。argv を JSON → base64 にして送り、
   インスタンス側の固定ランチャーが `execvp` で引数境界を復元する

2 があるので、仮に 1 を緩めてもリモートのシェルが値を解釈することはない。
`name_pattern` を広げた環境で 32 文字を超える名前を使いたい場合は `transport: api` を選ぶ。
